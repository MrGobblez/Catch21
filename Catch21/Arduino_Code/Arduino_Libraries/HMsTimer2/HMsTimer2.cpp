...
unsigned long HMsTimer2::hmsecs;
void (*HMsTimer2::func)();
volatile unsigned long HMsTimer2::count;
volatile char HMsTimer2::overflowing;
volatile unsigned int HMsTimer2::tcnt2;

void HMsTimer2::set(unsigned long hms, void (*f)()) {
	float prescaler = 0.0;
	...
	if ((F_CPU >= 1000000UL) && (F_CPU <= 16000000UL)) {	// prescaler set to 32
	...
	// Hard coded @ 0.1 ms for ATmega368!
	tcnt2 = 230;
...
void HMsTimer2::_overflow() {
	count += 1;
	
	if (count >= hmsecs && !overflowing) {
		overflowing = 1;
		count = 0;
		(*func)();
		overflowing = 0;
